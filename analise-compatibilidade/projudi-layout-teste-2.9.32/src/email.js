	...
	function ensureButtons() {
		...
		if (!emailMenuButton || !emailMenuButton.isConnected) {
			...
			emailMenuButton.addEventListener("click", function (event) {
				...
				if (emailDropdown && emailDropdown.isConnected) return closeEmailDropdown();
				emailDropdown = document.createElement("div");
				emailDropdown.id = "pdp-email-dropdown";
				emailDropdown.setAttribute("role", "menu");
				emailDropdown.style.cssText = "position:fixed;z-index:2147483001;padding:4px;border:1px solid #aaa;border-radius:4px;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.2);";
				const sender = document.createElement("button");
				sender.type = "button";
				sender.textContent = "Alterar Remetente";
				sender.setAttribute("role", "menuitem");
				sender.style.cssText = "display:block;width:100%;padding:7px 10px;border:0;background:#fff;text-align:left;cursor:pointer;";
				sender.addEventListener("click", function () { closeEmailDropdown(); openFromAccountsDialog(); });
