...
(function () {
	...
	// A tela de detalhes da movimentação apresenta ações administrativas
	// com a mesma classe e a mesma rota-base dos documentos. Elas não são
	// arquivos e não podem gerar painéis de Preview.
	const NON_DOCUMENT_LINK_LABEL = /^bloquear\s+por\b/i;
	...
	function collectDocsFrom(scope, baseURL) {
		...
		anchors.forEach(function (a) {
			...
			if (NON_DOCUMENT_LINK_LABEL.test(label)) return;
	...
	function isDocumentLink(el) {
		...
		if (href.indexOf(DOC_LINK_HREF_MARKER) === -1) return false;
		const label = (el.textContent || "").replace(/\s+/g, " ").trim();
		return !NON_DOCUMENT_LINK_LABEL.test(label);
	}
	...
	function loadedMovementDocs(link) {
		...
		if (!container) return [];
		return Array.prototype.slice.call(container.querySelectorAll('a.link[href*="/arquivo.do"]')).filter(isDocumentLink).map(function (docLink) {
			return {
	...
	function scheduleClosePendencia() {
		...
		closeTimerPendencia = setTimeout(function () {
			cleanupPendenciaLoader();
			closeAllPendenciaPanels();
		}, CLOSE_DELAY_MS);
	}
	...
	function showPendenciaMessage(link, message) {
		...
		panel.title.textContent = isMovementLink(link) ? "Movimentação" : "Pendências";
	...
	function openPendenciaGroup(link) {
		if (activePendenciaLink === link && (pendenciaPanels.length || movementMultipleNotice || (pendenciaLoader && pendenciaLoader.link === link))) {
			return;
		}
	...
	function openPreviewGroup(link) {
		...
		// invisível. O modo loader abaixo encontra os arquivos e devolve seus
	...
	document.addEventListener(
		...
		function (e) {
			...
			if (docLink) {
				...
				if (toEl && docLink.contains(toEl)) return;
			...
			if (groupLink) {
				const toEl = e.relatedTarget;
				if (toEl && groupLink.contains(toEl)) return;
	...
	// Nas rotas da blacklist de interface, mantém todo o código de Preview
	// acima ativo, mas não cria checkboxes, painel ou botão do WhatsApp.
